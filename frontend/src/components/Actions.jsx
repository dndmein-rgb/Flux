...
const Actions = ({post_ }) => {
  ...
  const handleLikeAndUnlike=async()=>{
    if(!user) showToast("Error","Please login to like a post","error");
    setIsLiking(true);
    try {
      ...
      if(!liked){
        ...
        setPost({...post, likes:post.likes.filter((userId)=>userId!==user._id)})
      }
      setLiked(!liked);
    } catch (error) {
  ...
  const handleReply=async()=>{
    setIsReplying(true);
    if(!user) showToast("Error", "Please login to reply to a post", "error");
    if(!reply) return;
    try {
    ...
    >
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme='blue' variant='outline' isLoading={isReplying} disabled={!reply} onClick={handleReply} size={'sm'} mr={3}>
              Reply
            </Button>
          </ModalFooter>
        </ModalContent>
